...
describe("Upcoming Virtual Visit Consultation", () => {
  ...
  it("Get The list the Upcoming Virtual Visit", () => {
    const currentDate = new Date();
    const futureDate = new Date(currentDate);
    futureDate.setDate(currentDate.getDate() + 1);

    const toDate = new Date(futureDate);
    toDate.setMonth(futureDate.getMonth() + 3);
    ...
    cy.fixture<{ GetUpcomingVituralVisitEndpoint: string }>(
      "apiEndpoints.json"
    ).then((apiEndpoints) => {
      const apiUrl: string =
        "https://consultation-api." +
        Cypress.env("env") +
        ".dctelemedplatform.com";
      const GetUpcomingVituralVisitEndpoint =
        apiEndpoints.GetUpcomingVituralVisitEndpoint;
      const url = `${apiUrl}${GetUpcomingVituralVisitEndpoint}`;
      cy.task("getAccessToken").then((token) => {
        if (!token) {
          throw new Error("Failed to retrieve access token");
        }
