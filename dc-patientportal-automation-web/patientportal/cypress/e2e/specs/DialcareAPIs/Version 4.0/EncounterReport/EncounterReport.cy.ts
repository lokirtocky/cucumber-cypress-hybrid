...
describe("Get Encounter Report Consultation", () => {
  ...
  it("Get Encounter Report Details", () => {
    cy.fixture<{ EncounterReportEndPoint: string }>("apiEndpoints.json").then(
      (apiEndpoints) => {
        const apiUrl: string =
          "https://consultation-api." +
          Cypress.env("env") +
          ".dctelemedplatform.com";
        const EncounterReportEndPoint = apiEndpoints.EncounterReportEndPoint;
        const url = `${apiUrl}${EncounterReportEndPoint}` + consultationId;
        cy.task("getAccessToken").then((token) => {
          if (!token) {
            throw new Error("Failed to retrieve access token");
          }
