...
describe("Getting the Email Details for cancel Appointment from Gmail...", () => {
  const apiUrl: string =
  ...
  it("Cancel Appointment API POST Request with English", () => {
    cy.task("runQuery", {
      secretName: SecretName.MemberDatabase,
      query: DatabaseQuery.UpdatePatientsPreferredEnLanguage + 133,
    }).then((result: any) => {
      cy.task("log", result);
    ...
    cy.fixture<{ CancelAppointmentEndPoint: string }>("apiEndpoints.json").then(
      (apiEndpoints) => {
        ...
        cy.task("getAccessToken").then((token) => {
          ...
          cy.log(`API URL: ${url}`);
          cy.log(`API Headers: ${JSON.stringify(headers)}`);
          cy.log(`API Payload: ${JSON.stringify(requestBodyEng)}`);
